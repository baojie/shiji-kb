...
- 🎮 **交互应用**：认知辅助阅读器（语法高亮+结构语义化）、事件地铁图、史记争霸游戏、15个可复用Skills
- 📊 **分析方法**：统计分析、网络分析、时空分析
...
从130章中提取结构化历史事件，构建事件知识网络：

- **已完成**：130章全覆盖事件识别（3,092个事件），11类事件类型
- **事件关系**：4,385条关系（自动2,198 + LLM 2,187），8种类型
- **跨章关联**：1,876条跨线换乘（互见294/共人867/共地542/同期173）
- **公元纪年**：782个事件标注公元年（前2700年～前87年），含世纪级和精确级
- **地铁图可视化**：130条线路 = 130篇，3,092个站点 = 3,092个事件，支持缩放/拖拽/搜索/实体链接
- **方法论**：`SKILL_事件识别.md` 沉淀了完整的事件识别方法和原则
