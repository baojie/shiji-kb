...
- 📋 **事件索引**：130章全覆盖事件识别（11类事件类型，含事件链分析）
...
- **已完成**：130章全覆盖事件识别，11类事件类型（战争、继位、政治活动、政治改革、政治整顿、家族事件、建设、文化活动、经济活动、自然灾害、改革）
- **事件要素**：事件ID、名称、类型、时间、地点、参与者、朝代、原文引用、段落位置
- **事件链**：每章梳理因果关系和时序关系的事件序列
- **方法论**：`SKILL_事件识别.md` 沉淀了完整的事件识别方法和原则
- **待完成**：跨章事件关联、事件知识图谱可视化、与实体索引双向链接
...
│   ├── extract_events.py         # 历史事件批量识别（API）
│   ├── validate_events.py        # 事件索引验证与统计
...
- 📦 **知识单元（SKU）体系**：由 anything2ontology 生成 434 个事实知识 + 241 个技能知识 + 关系知识，含中文索引文档
- 🏷️ **SKU 实体增补**：利用已有 10,110 个实体对 394 个 Factual SKU 进行实体匹配，生成 7,497 个实体标注
- ⏰ **时间线实体与年份消歧**：构建公元纪年映射，覆盖全部非年表章节
- 🔗 **命名实体索引系统**：11类实体索引页面（9,700+条目），正文实体可点击跳转到索引条目，别名合并（~570条），语义消歧（1,281处），拼音排序导航
