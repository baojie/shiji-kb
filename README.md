...
- 🏷️ **实体提取**：12,527个词条，71,857次标注（11类实体）
- 🔗 **实体索引**：11类实体索引页面（9,700+条目），正文实体可点击跳转，别名合并（~570条），语义消歧（1,281处），拼音排序导航
- 🕸️ **知识图谱**：人物、地名、官职、事件关系网络
- 📋 **事件索引**：130章全覆盖事件识别（3,092个事件，11类事件类型，含事件链分析）
- ⏱️ **事件年代标注**：3,051个事件标注公元纪年（98.7%覆盖），经两轮Agent反思审查修正1,441处，积累26条错误模式+12条推理逻辑
...
| [古籍人名消歧](SKILL_实体消歧.md) | 4层启发式策略解决同名异指（4,142人名，1,281处消歧） |
...
| **实体索引** | 11/11 (100%) ✅ | 11类实体索引页面，正文实体可点击跳转，别名合并（~570条），语义消歧（1,281处） |
| **实体词表** | ✅ 完成 | 提取12,527个词条，71,857次标注 |
...
│   │   ├── reports/             # 反思结果JSON（130章×2轮）+ pipeline状态
│   │   └── scripts/             # 提取/标注/质检/反思管线/可视化脚本
│   ├── entities/                 # 实体（12,527条 + 别名/消歧）
...
- **实体分布**：人名最多(4,142)，其次是官职(2,504)和地名(1,820)
...
- **年代审查**：经两轮Agent反思修正1,441处（第一轮1,010 + 第二轮431），积累SKILL规则26+12条
...
- ⏱️ **事件年代两轮反思审查**：Agent自动化反思管线对130章3,092个事件的年代标注进行两轮系统性审查，共修正1,441处（第一轮1,010 + 第二轮431），年代覆盖率从25%提升至98.7%（3,051/3,092），积累26条错误模式+12条推理逻辑写入SKILL文档
...
- 🔗 **命名实体索引系统**：11类实体索引页面（9,700+条目），别名合并（~570条），语义消歧（1,281处）
