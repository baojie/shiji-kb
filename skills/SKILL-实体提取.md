...
禁止标注的：纯代词（他/她/它/他们）、过于泛化的角色词（客户、财务、同事——除非上下文只有一位）。
...
1. 主语是"我"（不是别人的决定）
2. 原文有决定性表达：决定、打算、准备、要去、要做
3. 是具体可执行的行动（不是感想、评价、转述）
禁止标注的：别人的建议/决定（如"司机说走高架"）、模糊想法（想一想、也许、可能）、对事物的评价（"觉得还不错"）。

## question
说话人仍未解决的疑问或困惑。必须同时满足：
1. 主语是"我"（不是别人的问题）
2. 原文有疑问标志：不知道、不确定、怎么、为什么、能不能
3. 到文本结束时仍未被回答
禁止标注的：别人提出的问题、已经得到答案的问题、计划/打算（应归入decision）。
