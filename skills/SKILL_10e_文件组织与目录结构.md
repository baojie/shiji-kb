...
### ❌ 错误示例2：把原型HTML放在 `docs/`

```
docs/
└── prototype_v1.html  # 错误！这是实验性原型
```

**纠正**：
```
labs/prototypes/
└── 001_sanjia_sidebar.html
```

---

### ❌ 错误示例3：把临时脚本放在 `scripts/`

```
scripts/
└── test_pinyin.py  # 如果只是一次性测试
```
...
- [ ] 是否选择了正确的一级目录？
- [ ] 如果是 `docs/`，是否真的可以公开？
- [ ] 如果是 `labs/`，是否明确何时"毕业"到正式目录？
- [ ] 如果是 `scripts/`，是否已经过测试、可复用？
- [ ] 文件名是否语义清晰？
- [ ] 是否需要在README中说明此文件的用途？
