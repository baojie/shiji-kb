...
**方式B：内联消歧（v2.6新增，标注时直接写入全名）**
```
源文件(.tagged.md)  →  〖@台|吕台〗（显示"台"，全名"吕台"）
无需JSON            →  内联 | 后的全名优先级高于JSON消歧
渲染HTML            →  <span title="人名：吕台" data-canonical="吕台">台</span>
```
...
| 适用 | 章内一义的谥号（武王→周武王） | 单字省称（台→吕台）、段落内多义 |
