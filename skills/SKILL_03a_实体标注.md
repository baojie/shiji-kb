...
**v2.6 内联消歧语法（2026-03-16）**：
- 新语法 `〖@短名|全名〗`：显示短名，链接/tooltip/索引用全名
- 用途：单字省称（`〖@台|吕台〗`）、同名消歧（`〖@桓公|齐桓公〗`）
- 渲染：`<span class="person" title="人名：吕台" data-canonical="吕台">台</span>`
- 索引：以全名（`|`后）入索引，短名（`|`前）显示在正文
- 适用所有实体类型，优先级高于 disambiguation_map.json
