...
2. **13类〖TYPE〗标记** → `<span class="TYPE" title="类型名">`
3. **5类CJK括号标记**（〚〛/《》/〈〉/【】/〔〕）→ 对应 `<span>`
4. **引号** → `<span class="quoted">`（对话标识）
5. **段落编号** `[N.N]` → `<a class="para-num">` 可点击锚点
6. **实体链接** → 后处理包裹 `<a>` 指向实体索引页

**处理顺序**：粗体 → 器物/官职/地名等外层 → 人名（最后，常作内层）
