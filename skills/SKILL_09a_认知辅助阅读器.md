...
**设计文档**: [实体渲染规划 v5.0](../../doc/spec/实体渲染规划.md)

**核心理念**（v5.0）：
- **5色语义分组** + 下划线组合（实线/虚线/点线/波浪线/无）
- **统一淡黄底色** `rgba(255, 248, 220, 0.6)` - 与动词明确区分
...
| 类型 | CSS class | 颜色 | 下划线 | 特殊样式 |
|------|-----------|------|--------|---------|
| 人名 | `.person` | #8B4513 褐色 | 实线 | 淡黄底 |
| 地名 | `.place` | #B8860B 深金 | 实线 | 淡黄底 |
| 官职 | `.official` | #A0522D 赭褐 | 虚线 | 淡黄底 + 中粗 |
| **时间** | `.time` | **#008B8B 深青** | **点线** | **淡黄底（独立维度）** |
| 朝代 | `.dynasty` | #9370DB 中紫 | 实线 | 淡黄底 |
| 封国 | `.feudal-state` | #B266B2 梅红 | 实线 | 淡黄底 |
| 族群 | `.tribe` | #8B7BC8 淡紫 | 虚线 | 淡黄底 + 中粗 |
| 制度 | `.institution` | #4682B4 钢青 | 无 | 淡黄底 + 中粗 |
| 刑法 | `.legal` | #5F5FA0 深蓝紫 | 实线 | 淡黄底 |
| 礼仪 | `.ritual` | #6495ED 矢车菊蓝 | 点线 | 淡黄底 + 中粗 |
| 身份 | `.identity` | #4876A8 深钢青 | 虚线 | 淡黄底 |
| **典籍** | `.book` | **#2F4F4F 深灰绿** | **波浪线** | **淡黄底（传统书名线）** |
| 思想 | `.concept` | #3E6E5E 深青绿 | 点线 | 淡黄底 |
| 器物 | `.artifact` | #CD853F 秘鲁棕 | 无 | 淡黄底 + 中粗 |
| 生物 | `.biology` | #228B22 森林绿 | 虚线 | 淡黄底 + 中粗 |
| 天文 | `.astronomy` | #483D8B 深岩蓝 | 点线 | 淡黄底 |
| 神话 | `.mythical` | #8B008B 深品红 | 波浪线 | 淡黄底 + 中粗 |
| **数量** | `.quantity` | **#2E8B57 海绿** | **实线** | **等宽字体 + 加粗** |

**设计原则**:
- **颜色**：6色分组（时间独立），易记忆
- **下划线**：参考古籍专名线，实/虚/点/波浪区分
- **时间独立**：历史的第四维，青色象征时序流动
- **典籍波浪线**：传统书名号（《》）的延续
- **数量特殊**：参考IDE数字高亮，等宽字体便于对齐
- **tooltip提示**：悬停显示实体类型名，消歧后显示全名

### 3.2 动词渲染（4类动词实体）

**设计文档**: [动词渲染规则 v4.0](../../doc/spec/动词渲染规则.md)
...
2. **CSS样式设计**：
   - 实体渲染：见 §三.1 + [实体渲染规划 v5.0](../../doc/spec/实体渲染规划.md)
   - 动词渲染：见 §三.2 + [动词渲染规则 v4.0](../../doc/spec/动词渲染规则.md)
3. **批量生成**：`generate_all_chapters.py`（见 §二 批量生成）
4. **发布流水线**：`publish_to_docs.sh`（见 §七 发布流水线）
5. **质量检查**：`scripts/lint_html.py`（见 §七 步骤6）

**结论**：SKILL_03d 作为独立技能已废弃，所有功能并入 SKILL_09a 认知辅助阅读器。
