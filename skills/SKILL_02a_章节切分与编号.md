...
**父编号必需性（重要）**：
- **必须有一级编号才能有二级编号**：不允许出现 `[67.1]` 但缺少 `[67]` 的情况
- 若需要二级编号，必须先创建对应的一级编号段落
- 一级编号可以为空段落（只有编号，无内容），作为二级段落的容器
- 检查工具：`python scripts/check_missing_parent_pn.py`
- 修复工具：`python scripts/fix_missing_parent_pn.py`
