---
name: skill-butler-2
description: Butler 的原子行动目录 — 19 种标准操作的前置条件、执行步骤、后置检查、预期 diff 大小。每次 invocation 只执行 1 个原子操作, diff ≤ 20 行。accept 后只 git add 暂存，每17轮由 /wiki 批量 commit。禁止自由发挥, 只能从此目录挑。新操作由 W5 反思流程追加。
---
...
| `create-stub` | kg canonical 无 wiki 页 + 有 lifespan / refs | 跑 `generate_entity_page.py <name>` 生成内容，再用 `add_page.py` 写入（自动记录修订） | 页面 md 生成, pages.json 可扫到 | 新文件 50 行内 |
...
| `reclassify` | 页面 type 字段明显错误（W11 审计候选） | 改 frontmatter `type` 字段 | pages.json 更新 | 1 行 |
| `source-with-pn` | 页面 type 在 {person/concept/overview/skill/story} + 无 `sources`/`event_ids`/`pn` 字段 + canonical_name ≥ 3 字 | ① `python3 wiki/scripts/butler/find_unsourced.py --target <slug>` 确认匹配 event ② 用 `edit_page.py` 在 frontmatter 加 `sources:` 和 `event_ids:` ③ 正文加 `(NNN-MMM)` 行内 PN 引注 | frontmatter 含 `sources` + `event_ids`；引注 event_id 能在对应章节事件表中找到 | 3–6 行 |
| `delete-page` | 页面重复/错误/应合并 + 有用户或 W11 明确授权 | ① `record_revision.py <slug> --action delete --summary "..."` 留快照 ② `git rm wiki/public/pages/<slug>.md` ③ history JSON 保留不删 | history JSON 仍在且含 `deleted:true`；pages.json 重建后无此页 | 删文件 |
...
- ❌ **替换/覆盖既有 section**：即使新数据"更完整"，也不得用新内容覆盖已存在的节。
  节不存在 → 可追加新建；节已存在 → 必须跳过。去重/合并是后续工序，非本轮任务。
- ❌ 改 SKU 原文
- ❌ 改 kg/ 任何内容
- ❌ 一次动作同时写多个文件 (除了 `rebuild-registry` 的 `pages.json` + `backlinks.json`)
- ❌ **直接 Edit/Write wiki/public/pages/*.md**，必须通过：
...
11. 每17轮由 /wiki 批量 commit，commit message 汇总本批次所有 action
```
