---
name: skill-butler-2
description: Butler 的原子行动目录 — 23 种标准操作的前置条件、执行步骤、后置检查、预期 diff 大小与工作量单位(WU)。每次 invocation 按WU批量执行同类动作直到累计200WU，单个动作 diff ≤ 20 行。禁止自由发挥, 只能从此目录挑。新操作由 W5 反思流程追加。
---

# SKILL W2: 原子行动目录

> 每轮按"工作量单位(WU)"批量执行同类动作，目标 200 WU/轮。本 skill 是菜单——选一类，批量做。
...
- 每轮按 **WU** 批量执行同类动作，目标 200 WU/轮（见 PROMPT.md WU 表）
...
| `premium-upgrade` | 10 | 目标在精品页建设队列（W8/H7）+ 页面 ≥ 20 行基础内容 | 读 `SKILL_W8_精品页建设方法论.md`：① **必须**执行"步骤1b 跨章节互见搜索"——从页面 `## 史记引文` 的章节分布中，逐一扫描非本人专传章节原文，寻找他人对本人物的评语/夹叙/对照段落 ② 按最高价值来源（跨章节互见 → ontology v2 facts → 章节原文）深化一个维度 ③ 将发现的跨章节材料融入分析节，需含 PN 引注 | 新增内容有 PN 溯源；分析节包含 ≥ 1 处来自**非本人专传**章节的他人评价或夹叙；单次 diff ≤ 20 行 | 10–20 行 |
...
```
0. 查 WU 表，确定本轮 action 类型；batch_n = ceil(200 / WU)
...
2. 循环 batch_n 次（或队列同类耗尽）：
   ...
   f. 若 total_wu ≥ 200 或 consec_fail ≥ 3：退出循环
