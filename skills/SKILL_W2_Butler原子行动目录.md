---
name: skill-butler-2
description: Butler 的原子行动目录 — 18 种标准操作的前置条件、执行步骤、后置检查、预期 diff 大小。每次 invocation 只执行 1 个原子操作, 1 动作 = 1 commit, diff ≤ 20 行。禁止自由发挥, 只能从此目录挑。新操作由 W5 反思流程追加。
---
...
- 每动作 ↔ 1 个 git commit
- diff ≤ 20 行 (含空行), 超则拆
- commit message 格式：`butler/<action>: <target> <一句话>`
- 行动前必须写 `actions.jsonl` 一行
- 行动后必须跑 W4 评估
...
{"ts":"2026-04-22T14:00","mode":"trail","action":"add-link",
 ...
 "rationale":"...","result":"accept|fail|rollback","commit":"abc1234"}
```
...
| `rebuild-registry` | 前述动作涉及 frontmatter 变动 | 跑 `build_registry.py` | pages.json 注入 semantic 成功 | 1 文件 (pages.json 被覆写) |
...
- ❌ 改 SKU 原文
- ❌ 改 kg/ 任何内容
- ❌ 一次动作同时写多个文件 (除了 `rebuild-registry` 的 `pages.json`)
...
4. 在 actions.jsonl 追加一行 (result 先留空)
5. 做 diff
6. 检查 diff ≤ 20 行 (否则 rollback + 拆)
7. 跑 W4 评估
8. W4 accept → git commit; W4 fail → git restore + actions 改为 rollback
9. 更新 actions.jsonl 的 result 字段
10. 若需 pages.json 更新, 触发 rebuild-registry (单独 commit)
```
