---
name: skill-butler-2
description: Butler 的原子行动目录 — 19 种标准操作的前置条件、执行步骤、后置检查、预期 diff 大小。每次 invocation 只执行 1 个原子操作, diff ≤ 20 行。accept 后只 git add 暂存，每5轮由 /wiki 批量 commit。禁止自由发挥, 只能从此目录挑。新操作由 W5 反思流程追加。
---
