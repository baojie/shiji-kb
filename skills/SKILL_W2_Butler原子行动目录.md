...
| `premium-upgrade` | 10 | 目标在精品页建设队列（W8/H7）+ 页面 ≥ 20 行基础内容 | **以事实收集为主，降低分析比重**：① **必须**执行"步骤1b 跨章节互见搜索"——从页面 `## 史记引文` 的章节分布中，扫描非专传章节，寻找他人原文评语（带PN）② 优先新增**原文引文块**（生平事件证据、他人评价引文、太史公曰原文） ③ 若需补充分析，≤ 150 字陈述性说明，不写推理性长段 | 新增内容有 PN 引注；含 ≥ 1 处来自**非专传章节**的他人原文评价；分析文字 ≤ 原文引用字数；单次 diff ≤ 20 行 | 10–20 行 |
