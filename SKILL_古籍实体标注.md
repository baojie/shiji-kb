...
原始11类（v1.0）+ 2026-03-13扩展4类（v1.1）。
...
**v1.1扩展说明（2026-03-13）**：
- 原 `^制度^` 中混杂了典籍书名（如 `^春秋^`）和刑法（如 `^腰斩^`）→ 拆出为独立类型
- `《》` 专用于书名（原有中文书名号语义明确）
- `〈〉`/`【】`/`〔〕` 选用无歧义CJK标点
- `^制度^` 保留典章制度、礼法规范类词汇
...
ENTITY_PATTERNS = [
    (r'\*\*(.+?)\*\*', 'bold'),        # 1. 粗体必须先于 *器物*
    (r'〘(.+?)〙', 'flora-fauna'),      # 2. 动植物（CJK符号）
    (r'〚(.+?)〛', 'mythical'),         # 3. 神话（CJK符号）
    (r'《(.+?)》', 'book'),             # 4. 典籍（书名号）
    (r'〈(.+?)〉', 'ritual'),           # 5. 礼仪（CJK括号）
    (r'【(.+?)】', 'legal'),            # 6. 刑法（方括号）
    (r'〔(.+?)〕', 'concept'),          # 7. 思想（CJK括号）
    (r'\^(.+?)\^', 'institution'),      # 8. 制度
    (r'\$(.+?)\$', 'official'),         # 9. 官职
    (r'=(.+?)=', 'place'),             # 10. 地名
    (r'%(.+?)%', 'time'),              # 11. 时间
    (r'&(.+?)&', 'dynasty'),           # 12. 朝代
    (r'~(.+?)~', 'tribe'),             # 13. 族群
    (r'\*(.+?)\*', 'artifact'),        # 14. 器物（在粗体之后）
    (r'!(.+?)!', 'astronomy'),         # 15. 天文
    (r'@(.+?)@', 'person'),            # 16. 人名最后（常作内层嵌套）
]
```

**替换规则：**
- `**粗体**` 必须在 `*器物*` 之前处理
- `@人名@` 放最后（最常作为嵌套内层，如 `$@张良@$`）
- 每次替换后排除已生成的HTML标签内容
...
| 文件 | 位置 | 大小 | 说明 |
|------|------|------|------|
| `entity_index.json` | kg/entities/data/ | ~4.5MB | 11,606实体的全量索引（15类） |
...
| v2 | Markdown Token `@人名@` | 可读、可控、可扩展 |
| v3（当前） | Token + 外挂JSON元数据 | 源文件不变，消歧/年份通过元数据层增强 |
...
**v2→v3 的转折：** 发现消歧和年份映射如果直接修改源文件，会导致：
- 原文失真（"武王"变成"周武王"不是原文）
- 无法回溯修改
- 多人协作时频繁冲突
...
11. **渲染顺序至关重要** — `**粗体**` 和 `*器物*` 的冲突，`@人名@` 嵌套在 `$官职$` 内，都需要精心设计处理顺序。
...
*本SKILL文档基于《史记》130篇×57万字的实体标注实践（2025-02 至 2026-03）提炼。v1.1（2026-03-13）扩展为15类实体（+典籍/礼仪/刑法/思想）。*
*实际投入：~70小时人工 + ~5.7亿Token AI处理，产出11,606个实体、100,261次标注。*
