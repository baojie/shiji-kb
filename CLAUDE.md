...
详见宪法 §1.1。对**批量写入或导入操作**特别注意：

- ✅ 目标节/字段**不存在** → 可追加新建
- ✅ 目标字段**为空** → 可填入新值
- ❌ 目标节/字段**已存在** → **批量导入时必须跳过，不得覆盖**（单条精细编辑可提升内容）
- ❌ 用新数据整段替换旧节 → **禁止**
...
## pn 与 event_ids 互相独立（史记专属）

`pn`（段落引用号）与 `event_ids`（事件结构化编号）是两个完全独立的体系，修改一个时**不得联动修改**另一个：

- `pn`：原文段落引用号，格式 `（CCC-PPP）`，用于定位原文
- `event_ids`：事件结构化编号，格式 `[CCC-PPP]`，用于关联事件

血泪教训（2026-04-24）：将 pn `(097-3.1)` 改为 `(097-6)` 时错误地将 event_ids `097-003` 改成了 `097-006`，导致事件关联失准。
