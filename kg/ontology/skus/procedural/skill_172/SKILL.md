---
name: distant-friendship-near-attack-strategy
description: Use when developing a long-term territorial expansion strategy against multiple neighboring states.
---

# Distant Friendship, Near Attack Strategy (远交近攻)

A geopolitical doctrine for systematic territorial expansion that prioritizes conquering nearby territories while maintaining peaceful relations with distant powers.

## Core Principle
"Gain an inch and it is your inch; gain a foot and it is your foot" - territorial gains from nearby conquests are secure and defensible.

## Steps

1. **Identify the strategic center** - Target the geographic "hub" that controls access to other regions
2. **Befriend distant powers** - Establish diplomatic relations with states too far to directly threaten
3. **Isolate the target** - Ensure nearby targets cannot receive external support
4. **Conquer incrementally** - Take nearby territories piece by piece
5. **Consolidate before expanding** - Integrate each conquest before moving to the next
6. **Apply pressure on the center** - Use controlled territory to pressure remaining targets

## Warning Against Overreach
- Attacking distant enemies while bypassing neighbors is strategically flawed
- Distant conquests are difficult to supply and defend
- Conquering neighbors strengthens your base for future expansion

## Expected Outcomes
- Secure, contiguous territorial expansion
- Reduced risk of overextension
- Gradual dominance over the region
