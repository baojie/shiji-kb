---
name: diplomatic-persuasion-by-analogy
description: Use to dissuade an ally or adversary from continuing military action after a victory.
---

# Diplomatic Persuasion Through Strategic Analogy

A method to dissuade further military action by demonstrating that additional gains are impossible while risks are real.

## Overview
When a victorious general or ruler wants to continue attacking, use an analogy to show that further action offers no benefit but creates danger.

## Steps

1. **Acknowledge the achievement**
   - Recognize the victory and current status
   - Establish that the person has reached the highest position possible

2. **Present the analogy**
   - Use a memorable story that illustrates the principle
   - The "painting feet on a snake" analogy: someone won a wine-drinking contest by drawing a snake first, but lost the wine because he added unnecessary feet—making it no longer a snake

3. **Apply the analogy to the situation**
   - Show that further conquest adds no new reward
   - Demonstrate that failure would cost everything already gained

4. **Offer the alternative**
   - Suggest withdrawal with honor
   - Frame retreat as preserving gains and earning gratitude

## Key Principle
"When you are already at the top, you cannot go higher. Any further action can only result in loss."

## Expected Outcome
The target recognizes that continued action is irrational and withdraws while maintaining their position.
