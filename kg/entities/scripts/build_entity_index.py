...
ENTITY_TYPES = [
    ...
    ('time',        r'〖%([^〖〗\n]+)〗',  'time',        '时间',     'time.html'),
    ('dynasty',     r'〖&([^〖〗\n]+)〗',  'dynasty',     '氏族',     'dynasty.html'),
    ('feudal-state', r'〖◆([^〖〗\n]+)〗', 'feudal-state', '邦国',   'feudal-state.html'),
    ('institution', r'〖\^([^〖〗\n]+)〗', 'institution', '名物',     'institution.html'),
    ('tribe',       r'〖~([^〖〗\n]+)〗',  'tribe',       '族群',     'tribe.html'),
...
ENTITY_DESCRIPTIONS = {
    'person':       ('人名', '〖@〗', '史记中出现的历史人物，包括帝王将相、诸侯公卿、学者方士、刺客游侠等各类人物。既含有完整姓名，也含单字名、字、号及称谓。'),
    'place':        ('地名', '〖=〗', '史记中出现的地理名称，包括国都、城邑、山川、关隘、道路、地区等。涵盖中原及周边各地，时间跨度从上古至西汉。'),
    'official':     ('官职', '〖;〗', '史记中出现的官职名称与君主称号，包括中央百官、地方长吏、军职武衔，以及匈奴单于、外邦国王等各类政治职位与封号。'),
    'time':         ('时间', '〖%〗', '史记中出现的时间表述，包括纪年（年号、王年、岁名）、月份、季节及干支等历法用语。'),
    'feudal-state': ('邦国', '〖◆〗', '史记中出现的各类政治实体，包括三代邦国、春秋战国列国、秦汉统一王朝，以及西域、南越、朝鲜等外邦政权。强调其作为有组织的政治体（有君主、领土、制度）的属性。'),
    ...
    'institution':  ('名物', '〖^〗', '史记中出现的典章名物：政治/经济/军事制度（封建制、郡县制、井田制、察举制等）、礼乐规范（礼、乐、封禅、郊、明堂等）、律历名物（黄锺、十二律、正朔、闰馀等）、医学名物（膀胱、三焦、经络等）等专门术语。'),
...
def generate_type_page(type_key, css_class, label, filename, entries):
    ...
    # 类型定义说明
    if type_key in ENTITY_DESCRIPTIONS:
        _, marker, desc = ENTITY_DESCRIPTIONS[type_key]
        lines.append(f'<div class="entity-type-desc">'
                     f'<span class="type-label {css_class}">{label}</span>'
