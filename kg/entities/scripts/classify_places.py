...
OUT_CONF_JSON = _ROOT / 'kg' / 'entities' / 'data' / 'place_confidence.json'
...
EXPLICIT_REGION = {
    ...
    '淮南', '淮北',
    # 其他
    '河南江北', '河表',  # 注：'大河' 移至 EXPLICIT_WATER
    '西域', '南越地', '北边', '塞北', '塞外',
    '中县', '中国诸侯',
    ...
    '济东', '济西',
    # 注：'济北''济南' 已移至 HAN_VASSAL_KINGDOMS（诸侯王国，郡级）
    # 注：'西垂' 已移至 EXPLICIT_TOMB（秦先祖葬地）
    # 注：'大河' 已移至 EXPLICIT_WATER（= 黄河）
    '汉北', '汉南', '汉东', '汉西', '汉中',
    '淮东', '淮西', '淮南', '淮北', '淮泗', '淮海',
    '江南', '江北', '江东', '江西', '江左', '江右',
    '泗东', '泗西', '泗上',
...
EXPLICIT_PLAIN = {
    ...
    # 第五轮 Agent 深研确认
    '易下',            # 易水之下（樊哙击燕王臧荼战场）
    '三户',            # 漳水三户津（项羽渡漳战场）— 纠正原山脉
    '霸上',            # 白鹿原军队驻屯/郊祀高地（"沛公军霸上"）
}
...
EXPLICIT_NON_PLACE = {
    ...
    '寝', '观',       # 建筑类泛称
    '津',             # 津口泛称
    # 注：'阙'从误标移出——"斩使者于阙下"指宫阙（建筑），见下 EXPLICIT_BUILDING
}
...
EXPLICIT_MOUNTAIN = {
    ...
    '息壤',    # 神话山
    '嵩高',    # 嵩山别名（三家注"中岳嵩高也"）
    '甘泉',    # 甘泉山（三家注"甘泉，山名"）—— 兼宫名性质，主分类建筑
    '什谷', '横谷',   # 山谷
    # 注：'颍谷'从山脉移出—— 郑邑颍考叔所居，实为城邑（"颍谷之考叔"）
    '邓林',    # 山林
    ...
    '封',      # "封"可能指 封山
    '赤丽',    # 秦赵战场（可能 山地名）
    # 注：'庞' 移至 EXPLICIT_ANCIENT_CITY（楚邑"复雠、庞、长沙"并列）
    '封龙',    # 封龙山
    '太山',    # = 泰山别写
}
...
EXPLICIT_BUILDING = {
    ...
    # 第六轮补 (T1 抽样纠错)
    '阙',              # "斩使者于阙下"— 宫阙
    # 注：霸上移至原野（Agent 深研：白鹿原军队驻屯/郊祀地，非建筑）
}
...
EXPLICIT_TOMB = {
    '霸陵', '茂陵', '阳陵', '长陵', '杜陵', '平陵',
    '骊山陵', '始皇陵',
    # 注：'鸿'虽在"四大冢"列，但"冢皆有尝禾"指祭祀山，归山脉更妥
    # 秦先王葬地 / 汉初陵墓
    '西垂', '西山',             # 秦先祖葬地
    '衙', '嶓冢', '鸿冢',
...
HAN_VASSAL_KINGDOMS = {
    ...
    '郑国',
    # 注：下列原误列在此集合，实为 侯国（县级）或古方国，已移除
    #   '晋国','昌国','安国','充国','东国','南国','北国','西国','耆国'
    #   → 晋国/耆国 归国家；其他 → EXPLICIT_ANCIENT_CITY 或 HAN_XIAN
}
...
KNOWN_HAN_XIAN = {
    ...
    '符离', '江乘', '葭萌', '窳浑', '覃怀',
    '瑕丘',
    # 第五轮补 · 高 ref_count 且 汉书漏检 的战国+汉县
    # Agent 确认为县（稳定建置）
    '亢父', '宋子', '房子', '朝歌', '昌邑', '平阳', '曲沃', '少梁',
    '滑', '绛', '东垣', '鄗', '马陵', '六', '宁新中',
    '陉城', '番', '亳', '蒲', '乾侯',
    # 第六轮补：从 HAN_VASSAL_KINGDOMS 纠正过来的侯国（县级）
    '安国',    # 高祖功臣樊哙 等封侯
    '昌国',    # 战国乐毅封邑，汉韩信封侯
    '充国',    # 汉充国县
    '东国', '南国', '北国', '西国',   # 史记中散见小邑/侯国
    '颍谷',    # 郑邑（颍考叔所居）
    '安国',    # (重复，去重由 set 自动)
    # 战国邑，汉代未必独立县治，但 史记中多按邑出现 —— 归城邑（下面几条通过 EXPLICIT_ANCIENT_CITY 处理或后缀覆盖）
    # '武遂','新城','阏与' Agent 建议归城邑，从 HAN_XIAN 移除
    # 其他战国邑（仍需推导）
    '几', '垣雍', '会甀', '皮牢', '羁马', '灵壁', '煮枣', '棘壁',
    '阴晋', '中人', '雒邑',
    '蓝田',    # 汉蓝田县（兼蓝田山，多标签）
    # 沛郡一带（刘邦起家）
...
def main():
    ...
    # 同步输出每个 (name, cat) 的置信度，供 HTML 渲染显示
    # 延迟导入 confidence_report 以避免循环依赖
    try:
        import confidence_report as cr  # type: ignore
        # 构建 peer map
        peer_map = defaultdict(set)
        for g in peer_groups:
            for n in g:
                peer_map[n].update(x for x in g if x != n)
        all_places_set = set(places.keys())

        conf_data = {}
        for name, cats in result.items():
            refs = places[name].get('refs') if isinstance(places[name], dict) else None
            conf_row = {}
            for cat in cats:
                ev = cr.score_evidence(name, cat, refs, hints,
                                        cp_mod=sys.modules[__name__],
                                        all_places=all_places_set,
                                        peer_map_cache=peer_map)
                conf_row[cat] = round(max((e[1] for e in ev), default=0.30), 2)
            conf_data[name] = conf_row
        with open(OUT_CONF_JSON, 'w', encoding='utf-8') as f:
            json.dump(conf_data, f, ensure_ascii=False, indent=2)
        print(f'Written: {OUT_CONF_JSON}')
    except Exception as e:
        print(f'[warn] 置信度输出失败: {e}')
