...
CAT_GENERIC  = '泛称'
CAT_JIACHEN  = '家臣'
CAT_JIBIE    = '级别'   # 秩禄/位次通称，非具体官职（如 二千石、长吏、百石）
CAT_MIS      = '误标'
CAT_PERSON_MIS = '人名误标'
...
SUFFIX_RULES = [
    ...
    # 单字通称
    # 注：X公 启发式已废除（古爵分类取消；X公 多指具体诸侯君，走人名消歧）
    ...
    # 注：X伯 启发式已废除（古爵分类取消；方伯/侯伯 已归泛称，西伯/郑伯 已归人名）
...
def classify_l1_whitelist(name):
    ...
    if name in EXPLICIT_JIBIE:     cats.append(CAT_JIBIE)
