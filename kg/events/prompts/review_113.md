...
你的任务是审查并修正《史记》113_南越列传的事件索引中的年代标注。
...
1. **事件索引**（待审查和修改）：`kg/events/data/113_南越列传_事件索引.md`
...
### 第三步：读取事件索引
完整读取 `kg/events/data/113_南越列传_事件索引.md`，统计事件总数，识别锚点分布（有多少个精确纪年事件、推断纪年事件、无纪年事件）。
...
| 113-006 | 赵佗役属闽越西瓯 | 政治活动 | %高后%崩后 （公元前180年） | =闽越=、=西瓯=、=骆= | @赵佗@ | &汉& |
...
- **段落位置**: [2]
- **年代推断**: 原文明记"高后崩"后，吕后崩于前180年（高后八年）
