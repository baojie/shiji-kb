...
你的任务是审查并修正《史记》016_秦楚之际月表的事件索引中的年代标注。
...
1. **事件索引**（待审查和修改）：`kg/events/data/016_秦楚之际月表_事件索引.md`
...
### 第三步：读取事件索引
完整读取 `kg/events/data/016_秦楚之际月表_事件索引.md`，统计事件总数，识别锚点分布（有多少个精确纪年事件、推断纪年事件、无纪年事件）。
...
- **Claude反思修正**：在 001, 003, 004, 005, 010, 009, 008, 002, 013, 015 中出现
...
| 016-005 | 陈涉死 | 战争 | %二世元年十二月% （公元前209年） | - | @陈涉@ | &楚& |
...
- **建设**: 0
- **自然灾害**: 0
- **年代推断**: 原文记载汉五年二月，即公元前202年

---

## 关键事件链

1. **反秦起义** (016-001 → 016-002 → 016-003 → 016-005 → 016-006)
   - 陈涉起兵 → 项梁号武信君 → 沛公初起 → 陈涉死 → 项梁立楚怀王

2. **项羽崛起** (016-007 → 016-008 → 016-009 → 016-011 → 016-012)
   - 项梁败死 → 项羽矫杀宋义 → 巨鹿之战 → 坑秦降卒 → 分封诸侯

3. **楚汉争霸** (016-010 → 016-014 → 016-015 → 016-016 → 016-017 → 016-018 → 016-019)
   - 沛公入咸阳 → 汉王伐楚至彭城 → 项羽大败汉军 → 韩信灭赵 → 韩信定齐 → 杀项籍天下平 → 汉王即帝位
...
- **建设**: 0
- **自然灾害**: 0
- **年代推断**： 年份本身正确（年表前202年条明确记载'二月，汉王即皇帝位'），但详情section缺少年代推断行。该推断行被错误放置到文件末尾统计信息区域（line 253：'年代推断: 原文记载汉五年二月，即公元前202年'）。属于已知错误模式23（年代推断行位置错位至非事件区域）。需将该行移回016-019详情的段落位置之后，并删除统计信息区域中的错误行。
