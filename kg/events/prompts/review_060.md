...
你的任务是审查并修正《史记》060_三王世家的事件索引中的年代标注。
...
1. **事件索引**（待审查和修改）：`kg/events/data/060_三王世家_事件索引.md`
...
### 第二步：建立时代框架
本章为通论性质（三王世家），时间跨度可能从远古到汉武帝，横跨多个时代。
**重点检查**：
...
### 第三步：读取事件索引
完整读取 `kg/events/data/060_三王世家_事件索引.md`，统计事件总数，识别锚点分布（有多少个精确纪年事件、推断纪年事件、无纪年事件）。
...
- **Claude反思修正**：在 001, 003, 007, 004, 005, 010, 009, 002, 008, 014, 013, 011, 019, 016, 015, 025, 012, 023, 030, 024, 026, 029, 034, 035, 032, 036, 028, 033, 037, 038, 039, 040, 041, 042, 043, 044, 045, 046, 047, 048, 049, 050, 052, 056, 057, 058, 059 中出现
...
| 060-017 | 燕王旦请宿卫被斥 | 政治活动 | [约公元前87年] | - | @旦@、@武帝@ | &汉& |
...
- **段落位置**: 褚先生曰
- **年代推断**: 约前87年，戾太子死于前91年巫蛊之祸后、武帝崩（前87年）前，旦上书请宿卫
...
- **原文引用**: "旦使来上书，请身入宿卫於=长安=。@孝武@见其书，击地，怒曰：'生子当置之=齐=鲁^礼义^之乡，乃置之燕=赵=，果有争心，不让之端见矣。'於是使使即斩其$使者$於阙下。"
- **段落位置**: 褚先生曰
- **年代推断**: 戾太子死于征和二年（前91年）巫蛊之祸，旦上书请宿卫在太子死后、武帝崩（前87年后元二年）之前。据《汉书·武五子传》，此事约在后元元年（前88年）前后。标注前87年过晚（前87年武帝已病重并于二月崩），约前89年更合理。
...
- **家族事件**: 2
- **政治整顿**: 2
- **年代推断**: 年份正确，但详情部分缺少'年代推断'行。应补充：本始元年（前73年），原文明确记载'以本始元年中'。此外，统计信息部分的'年代推断'字段位置不当，应移至详情的060-021条目下。
