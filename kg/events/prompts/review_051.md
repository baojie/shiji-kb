...
你的任务是审查并修正《史记》051_荆燕世家的事件索引中的年代标注。
...
1. **事件索引**（待审查和修改）：`kg/events/data/051_荆燕世家_事件索引.md`
...
### 第二步：建立时代框架
本章涉及秦末楚汉（荆燕）时期。参照 `reign_periods.json` 和 `中国历史大事年表.md` 中的相关数据。

### 第三步：读取事件索引
完整读取 `kg/events/data/051_荆燕世家_事件索引.md`，统计事件总数，识别锚点分布（有多少个精确纪年事件、推断纪年事件、无纪年事件）。
...
| 051-004 | 刘贾击临江王 | 战争 | %五年% [约公元前202年] | =临江= | @刘贾@、@卢绾@、@共尉@ | &汉& |
...
- **段落位置**: [3]
- **年代推断**: 章节插值：前锚051-003(前202年)，后锚051-005(前201年)，取前锚
