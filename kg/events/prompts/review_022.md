...
你的任务是审查并修正《史记》022_汉兴以来将相名臣年表的事件索引中的年代标注。
...
1. **事件索引**（待审查和修改）：`kg/events/data/022_汉兴以来将相名臣年表_事件索引.md`
...
### 第三步：读取事件索引
完整读取 `kg/events/data/022_汉兴以来将相名臣年表_事件索引.md`，统计事件总数，识别锚点分布（有多少个精确纪年事件、推断纪年事件、无纪年事件）。
