# SKILL: 古籍人名消歧 — 从短称到唯一指称

> 从《史记》130篇4142个人名的消歧实践中提炼，适用于古籍中同名异指的自动化解决。
...
**数据规模**：《史记》130篇中有81个歧义短名（如"武王"、"桓公"、"惠王"），涉及261条（短名, 国名）→全名的映射规则。当前已消歧329条章节级映射，覆盖53个章节。

### 1.2 核心约束

**不修改源文件** — 消歧结果作为独立JSON元数据，渲染时注入。原文中"武王"保持不变，在HTML中通过tooltip显示"周武王"、链接指向正确实体。
...
| 短名 | 出现章节数 | 未覆盖章节数 | 示例未覆盖章节 |
|------|-----------|-------------|----------------|
| 武王 | 32 | 12 | 013, 017, 024, 025, 044... |
| 惠王 | 22 | 10 | 034, 046, 063, 065, 066... |
| 文王 | 19 | 9 | 013, 041, 047, 050, 061... |
| 桓公 | 14 | 6 | 032, 033, 034, 046, 086... |
| 昭王 | 23 | 5 | 032, 034, 066, 074, 078 |
...
| 数据文件 | 位置 | 说明 |
|----------|------|------|
| `disambiguation_map.json` | kg/entities/data/ | 章节级消歧映射（53章329条） |
| `entity_aliases.json` | kg/entities/data/ | 别名→规范名（~570条） |
| `entity_index.json` | kg/entities/data/ | 全量实体索引（12,527实体） |

---

*本SKILL文档基于《史记》130篇×4,142个人名的消歧实践（2025-02 至 2026-03）提炼。*
*当前覆盖率：53/130章节，329条映射。待扩展至全部130章。*
