...
| `pn-citation` | `（NNN-MMM）` 引文链接 | onAfterRender |
| `backlinks` | 页面底部反向引用列表 | onBoot, onAfterRender |
| `place-map` | 地名/邦国 sidebar 地图（Leaflet + CHGIS 坐标） | onInfobox |
...
| `recent.jsonl` | 最近更新列表（JSONL，滚动窗口 1000 条） | `python wiki/scripts/record_revision.py` |
...
```
fetch pages.json (原 2.6 MB)
  ↓
for-loop: await import plugin × 4（串行）
  ↓
fetch 页面 .md → render
```
...
| 优化项 | 文件 | 效果 |
|--------|------|------|
| `pages.json` minify（去缩进/空格）| `build_registry.py` | 1.96 MB → 1.42 MB（-27%） |
| `pages.json` gzip 动态压缩 | `serve.py` | 1.96 MB → **224 KB**（-89%） |
| 去除 `_score_parts` 字段 | `build_registry.py` | 2.61 MB → 1.96 MB（-25%） |
| 插件并行加载 | `core.js` `loadPlugins()` | 串行 4 次 await → `Promise.all` |

**合计：首屏 JSON 传输量从 2.6 MB 降至 224 KB（-91%）。**

### pages.json 字段说明

当前保留字段：`type` / `label` / `aliases` / `tags` / `featured` / `path` / `quality_score`

`_score_parts` 仅在构建调试时有用，已从输出中移除。如需查看各分项，在 `build_registry.py` 中临时加回即可。

### 格式选型备忘

| 格式 | 原始大小 | gzip 后 | 备注 |
|------|---------|---------|------|
| pretty JSON | 1.96 MB | 250 KB | 原始格式 |
| compact JSON | 1.42 MB | 224 KB | **当前** |
| YAML block | 1.55 MB | 232 KB | 比 JSON 大，且需引入 js-yaml |
| YAML flow | 1.41 MB | 244 KB | gzip 后反而更大 |

结论：compact JSON + gzip 是最优组合，YAML 无收益。
